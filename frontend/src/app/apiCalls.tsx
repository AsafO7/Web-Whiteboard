...
export const loginUser = async (user: { name: string | undefined, email: string | undefined, password: string | undefined }) => {
    try {
        const response = await axios.post("172.105.130.95:5000/lobby", user)
...
export const logoutUser = async (userInfo: { name: String, email: String, currentRoom: String }) => {    
    try {
        const response = await axios.post("172.105.130.95:5000/", userInfo)
        return response.data
    }
    catch(err) {
        console.log(err)
...
export const updateUserRoom = async ( userInfo: { name: String, email: String, currentRoom: String }) => {
    try {
        const response = await axios.put("172.105.130.95:5000/room", userInfo)
        return response.data
    }
    catch(err) {
        console.log(err)
...
export const getLobbyInfo = async (user: { name: String, email: String, currentRoom: String }) => {
    try {
        const response = await axios.get("172.105.130.95:5000/lobby", { params: user })
        return response.data
    }
    catch(err) {
        console.log(err)
...
export const createRoom = async (userName: String, roomName: string | undefined, roomId: String) => {
    try {
        const response = await axios.post("172.105.130.95:5000/room", {userName, roomName, roomId})
        return response.data
    }
    catch(err) {
        console.log(err)
...
export const getRoomInfo = async (roomId: String, username: String) => {
    ...
    try {
        const response = await axios.get(`172.105.130.95:5000/room/:${roomId}`, { params: params })
        return response.data
    }
    catch(err) {
        console.log(err)
